...
class Fido2 {
  ...
  async registration(username: string, displayName: string, id: string) {
    ...
    registrationOptions.challenge = base64.fromArrayBuffer(
  ...
  async attestation(
    clientAttestationResponse: string,
    origin: string,
    challenge: string,
  ) {
  ...
  async login() {
    ...
    assertionOptions.challenge = base64.fromArrayBuffer(
  ...
  async assertion(
    ...
    expectedAssertionResult: IAssertionExpectations,
  ) {
